jQuery(document).ready(function ($) {
  ...
  function changeHeight(myClass) {
    const height = $(myClass).data("height");
    const heightMobile = $(myClass).data("heightmobile");
    const heightTablet = $(myClass).data("heighttablet");
    const heightDesktop = $(myClass).data("heightdesktop");
    const videoRatio = $(myClass).data("ratio");
    const ratio =
    $(".minimalio-video-banner__frame").innerWidth() /
      $(".minimalio-video-banner__frame").innerHeight();

    function changeHeight() {
      if (height == "custom") {
        if ($(window).width() > 992) {
          $(myClass)
            .find(".minimalio-video-banner__frame")
            .css("height", heightDesktop + "vh");
        } else if ($(window).width() < 992 && $(window).width() > 767) {
          $(myClass)
            .find(".minimalio-video-banner__frame")
            .css("height", heightTablet + "vh");
        } else {
          $(myClass)
            .find(".minimalio-video-banner__frame")
            .css("height", heightMobile + "vh");
        }
      }
        if (videoRatio === "16-9") {
          if (ratio > 1.77) {
  ...
  changeHeight(".minimalio-video-banner");
  window.onresize = function() {
    changeHeight(".minimalio-video-banner");
  }
  setTimeout(changeHeight(".minimalio-video-banner"), 100);
  ...
  $(".minimalio-video-banner").each(function () {
    const autoplay = $(this).data("autoplay");
    const myControls = $(this).data("controls");
    ...
    YoutubeBackground.prototype.injectIFrame = function () {
      ...
      this.iframe.setAttribute("class", "youtube-iframe");
      this.iframe.setAttribute("allow", ["autoplay; mute"]);
      var src =
        "https://www.youtube.com/embed/" +
        this.ytid +
        "?enablejsapi=1&disablekb=1&controls=0&rel=0&iv_load_policy=3&cc_load_policy=0&playsinline=1&showinfo=0&fs=0&origin=" +
    ...
    YoutubeBackground.prototype.buildHTML = function () {
      ...
      if (this.params["play-button"] || this.params["mute-button"]) {
        ...
        controls.className = "video-background-controls " + myControls;
    ...
    YoutubeBackground.prototype.generateActionButton = function (obj) {
      ...
      btn.style.background = $(".minimalio-video-banner").data("controls-color")
